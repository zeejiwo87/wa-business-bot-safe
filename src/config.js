...
module.exports = {
  ...
  bmkgDefaultKelurahan:
    process.env.BMKG_DEFAULT_KELURAHAN || 'Tamansari',

  bmkgDefaultAdm4:
    process.env.BMKG_DEFAULT_ADM4 || '35.07.06.2012',

  payment: {
    gopayNumber: (process.env.GOPAY_NUMBER || '085876846768').replace(/\D/g, ''),
    gopayName: process.env.GOPAY_NAME || 'Ahmad Fauzy',

    seabankNumber: (process.env.SEABANK_NUMBER || '901199544254').replace(/\D/g, ''),
    seabankName: process.env.SEABANK_NAME || 'Ahmad Fauzy',
  },

  tz: process.env.TZ || 'Asia/Jakarta',

  logOrderMessages:
    String(process.env.LOG_ORDER_MESSAGES || 'true').toLowerCase() === 'true',
};